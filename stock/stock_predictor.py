...
class StockDataset(Dataset):
    ...
    def __getitem__(self, idx):
        return self.sequences[idx].to(device), self.targets_reg[idx].to(device), self.targets_cls[idx].to(device)
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            for sequences, targets_reg, targets_cls in val_loader:
                ...
                all_cls_preds.extend(torch.argmax(cls_pred.cpu(), dim=1).numpy())
                all_reg_targets.extend(targets_reg.cpu().numpy())
                all_cls_targets.extend(targets_cls.cpu().numpy())

                running_corrects_cls_preds += torch.sum(preds == targets_cls.data)
            
        print(running_corrects_cls_preds.double()/len(val_loader.dataset))
        # 计算平均验证损失
...
def main():
    # 读取数据（替换为你的数据路径）
    df = pd.read_csv('/home/chen_c6/fightMoney/stock/out19.csv')
    # 定义特征列
    ...
    X_train, X_temp, y_reg_train, y_reg_temp, y_cls_train, y_cls_temp = train_test_split(
        sequences, targets_reg, targets_cls, test_size=0.4, shuffle=False  # 时间序列不打乱
    )
